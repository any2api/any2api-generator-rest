...
var updateSpecRamlSync = function(args) {
  ...
  // Parameters schema
  var paramsSchema = _.cloneDeep(invokerJson[invokerName].parameters_schema);

  if (executable) _.extend(paramsSchema, executable.parameters_schema);

  var paramsRequired = invokerJson[invokerName].parameters_required || [];

  if (executable) paramsRequired.concat(executable.parameters_required || []);
  ...
  // Results schema
  var resultsSchema = _.cloneDeep(invokerJson[invokerName].results_schema);

  if (executable) _.extend(resultsSchema, executable.results_schema);

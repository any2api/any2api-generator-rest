...
var updateSpecRamlSync = function(args) {
  ...
  if (executable) {
    ...
    var invokerDocs = {
      ...
      content: 'Use invoker to run arbitrary, supported executables in an ad-hoc manner.'
    };
  ...
  if (invoker && !executable) {
    customSchema.properties.executable = executableSchema;
  }
  ...
  // Generate examples
  var reqExample = JSON.stringify(generateExample({ type: 'request',
                                                    parameters_schema: parameters_schema,
                                                    parameters_required: parameters_required }), null, 2);
  var resExample = JSON.stringify(generateExample({ type: 'response',
                                                    parameters_schema: parameters_schema,
                                                    parameters_required: parameters_required }), null, 2);

  customRoute.post.body['application/json'].example = reqExample;
  customRoute.post.responses['201'].body['application/json'].example = resExample;

  customRoute['/{runId}'].put.responses['200'].body['application/json'].example = resExample;
  customRoute['/{runId}'].get.responses['200'].body['application/json'].example = resExample;
};



var generateExample = function(args) {
  var type = args.type; // request, response
